...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (
      !refundRequestId ||
      !["approve_refund", "approve_rebook", "reject"].includes(decision)
    ) {
      return new Response(
        JSON.stringify({
          error: "refundRequestId and valid decision required",
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (decision === "reject" && hostNote.trim().length < 10) {
      return new Response(
        JSON.stringify({ error: "Rejection reason required (min 10 chars)" }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    if (rr.host_id !== userId)
      return new Response(JSON.stringify({ error: "Not allowed" }), {
        status: 403,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    if (rr.status !== "pending_host")
      return new Response(JSON.stringify({ error: "Already decided" }), {
        status: 409,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    ...
    const { data: property } = await admin
      .from("properties")
      .select("title")
      .eq("id", rr.property_id)
      .maybeSingle();

    const invoke = (payload: Record<string, unknown>) =>
      admin.functions
        .invoke("send-transactional-email", { body: payload })
    ...
    if (decision === "approve_refund") {
      ...
      const guestMsg =
        ...
        fmtIQD(refundAmount) +
        " for booking number " +
        shortId(rr.booking_id) +
        " has been approved by the host. Please allow 14 days to receive your refund before contacting our team about your refund.";
      ...
      if (guestProfile?.email)
        invoke({
          templateName: "refund-approved-guest",
      ...
      const hostMsg =
        "Thank you for approving the refund request for booking number " +
        shortId(rr.booking_id) +
        ", the guest will now be refunded for the booking.";
      await admin.from("notifications").insert({
        user_id: rr.host_id,
        title: "Refund approved",
        message: hostMsg,
      ...
      if (hostProfile?.email)
        invoke({
          templateName: "refund-approved-host",
      ...
      const rebookMsg =
        "Hi — regarding your refund request for booking #" +
        shortId(rr.booking_id) +
        ", I'd like to offer to reschedule instead of a refund.\n\nBooking amount (" +
        fmtIQD(excFee) +
        ") has been made into a discount code for you, with a 1 month expiry to use only at this property.\n\n" +
      ...
      if (guestProfile?.email)
        invoke({
          templateName: "rebook-offered-guest",
          recipientEmail: guestProfile.email,
