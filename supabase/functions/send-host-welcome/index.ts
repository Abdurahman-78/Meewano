...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!resendApiKey) {
      await supabase.from('email_send_log').insert({
        message_id: messageId, template_name: 'host_welcome', recipient_email: email, status: 'failed',
    ...
    const resendResponse = await fetch('https://api.resend.com/emails', {
      ...
      headers: {
        ...
        'Idempotency-Key': `host-welcome-${email}`,
      },
      body: JSON.stringify({
        ...
        html, text,
        tags: [{ name: 'template', value: 'host_welcome' }],
      }),
    })
    ...
    await supabase.from('email_send_log').insert({
      message_id: messageId, template_name: 'host_welcome', recipient_email: email, status: 'sent',
    })
