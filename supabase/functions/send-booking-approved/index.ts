...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!resendApiKey) {
      await supabase.from('email_send_log').insert({
        message_id: messageId, template_name: 'booking_approved', recipient_email: email, status: 'failed',
    ...
    const resendResponse = await fetch('https://api.resend.com/emails', {
      ...
      headers: {
        ...
        'Idempotency-Key': `booking-approved-${booking.confirmationNumber}`,
      },
      body: JSON.stringify({
        ...
        html, text,
        tags: [{ name: 'template', value: 'booking_approved' }],
      }),
    })
    ...
    await supabase.from('email_send_log').insert({
      message_id: messageId, template_name: 'booking_approved', recipient_email: email, status: 'sent',
    })
