...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: isMod } = await admin.rpc("has_role", {
      _user_id: userId,
      _role: "moderator",
    });
    if (!isAdmin && !isMod)
    ...
    if (
      !refundRequestId ||
      !["approve_rejection", "decline_rejection"].includes(decision)
    ) {
      return new Response(
        JSON.stringify({
    ...
    if (
      decision === "decline_rejection" &&
      ![100, 75, 50, 25].includes(refundPct)
    ) {
      return new Response(
        JSON.stringify({ error: "refundPct must be 100, 75, 50 or 25" }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    if (rr.status !== "host_rejected")
      return new Response(
        JSON.stringify({ error: "Not in host_rejected state" }),
        {
          status: 409,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    if (decision === "approve_rejection") {
      ...
      const msg =
        "Unfortunately, your refund request under exceptional circumstances for booking number " +
        shortId(rr.booking_id) +
        " has been declined by the host.";
      ...
      if (guestProfile?.email)
        invoke({
          templateName: "refund-rejected-guest",
      ...
      if (guestProfile?.email)
        invoke({
          templateName: "refund-admin-override-guest",
          recipientEmail: guestProfile.email,
      ...
      if (hostProfile?.email)
        invoke({
          templateName: "refund-admin-override-host",
