...
type SortOption =
  "recommended" | "price-low" | "price-high" | "rating" | "recent";
...
const SearchResults = () => {
  ...
  const {
    ...
    (properties || []).forEach((p) => {
      ...
      if (t) typeSet.add(t);
      RATING_THRESHOLDS.forEach((r) => {
        if ((p.rating || 0) >= r) ratings.add(r);
      });
    ...
    return {
      ...
      availablePropertyTypes: PROPERTY_TYPE_KEYWORDS.filter((t) =>
        typeSet.has(t),
      ),
  ...
  const bedroomOptions = useMemo(() => {
    ...
    for (let i = 1; i <= cap; i++)
      opts.push(i === cap && maxBedrooms > cap ? `${i}+` : String(i));
    return opts;
  }, [maxBedrooms]);
  ...
  const hasChanges =
    ...
    JSON.stringify(pendingPropertyTypes) !==
    ...
    (pendingDateRange?.from?.toISOString() || "") !==
      (selectedDateRange?.from?.toISOString() || "") ||
    (pendingDateRange?.to?.toISOString() || "") !==
  ...
  const filteredAndSortedProperties = useMemo(() => {
    ...
    if (selectedBedrooms && selectedBedrooms !== "Any") {
      ...
      if (!isNaN(n)) {
        filtered = filtered.filter((p) =>
          isPlus ? (p.bedrooms || 0) >= n : (p.bedrooms || 0) === n,
        );
      }
    }
    ...
    if (selectedDateRange?.from && selectedDateRange?.to) {
      ...
      filtered = filtered.filter((p) => {
        if (
          (p as any).available_from &&
          new Date((p as any).available_from) > from
        )
          return false;
        if ((p as any).available_to && new Date((p as any).available_to) < to)
          return false;
        const blocked = ((p as any).blocked_dates as string[]) || [];
        if (blocked.length === 0) return true;
        const blockedSet = new Set(
          blocked.map((d) => new Date(d).toDateString()),
        );
                  ...
                  <div className="flex items-start gap-2">
                    <div className="flex-1">
                      <Input
                        ...
                        onChange={(e) =>
                          setPendingMinPrice(parseIQD(e.target.value))
                        }
                      />
                    </div>
                    ...
                    <div className="flex-1">
                      <Input
                        ...
                        onChange={(e) =>
                          setPendingMaxPrice(parseIQD(e.target.value))
                        }
                      />
                    </div>
                  </div>
                          ...
                          {bedroomOptions.map((opt) => {
                            ...
                            return (
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  setPendingBedrooms(opt === "Any" ? "" : opt)
                                }
                        ...
                        <PopoverTrigger asChild>
                          ...
                          >
                            ...
                            {pendingDateRange?.from ? (
                              pendingDateRange.to ? (
                                <>
                                  {format(pendingDateRange.from, "LLL d")} –{" "}
                                  {format(pendingDateRange.to, "LLL d, yyyy")}
                                </>
                              ) : (
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-muted-foreground">
                    {isLoading
                      ? "Loading..."
                      : `${filteredAndSortedProperties.length} ${t("propertiesFound")}`}
                  </p>
                </div>
