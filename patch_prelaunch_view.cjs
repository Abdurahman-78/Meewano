...
// replace title logic
const titleLogic = `  const registeredCount = realProperties.length;
...
const newTitleLogic = `  const { t } = useTranslation();
...
// update the subtitle
code = code.replace(
  /<p className="text-sm sm:text-base text-muted-foreground mt-2 max-w-xl">\s*All registered properties will be advertised during the main website launch\. Be from the first 100 hosts to register your property and receive one time only special offer\.\s*<\/p>/g, 
